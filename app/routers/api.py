...
@router.post("/products/{sku}/ai-rich/publish")
async def product_ai_rich_publish(
    sku: str,
    request: Request,
    user: User = Depends(require_user),
    db: AsyncSession = Depends(get_db),
):
    """Публикует Rich Content JSON в карточку товара на Ozon (Seller API).

    Перезаписывает текущее описание — подтверждение делается в интерфейсе.
    """
    from app.models import ProductInfo
    from app.services.seller_sync import get_active_seller_client
    from app.services.seller_client import SellerAPIError

    body = await request.json()
    rich = body.get("rich")
    plain_text = (body.get("description") or "").strip()
    if not rich or not isinstance(rich, dict) or not rich.get("content"):
        raise HTTPException(400, "Пустой rich_content_json")
    ...
    seller = await get_active_seller_client(db, user.id)
    if seller is None:
        raise HTTPException(400, "Ключи Seller API не подключены (Настройки)")

    # Текстовая версия описания из блоков rich (fallback для поля description)
    if not plain_text:
        parts = []
        for block in rich.get("content", []):
            if block.get("widgetName") == "raTextBlock":
                parts.extend(block.get("text", {}).get("content", []))
            elif block.get("widgetName") == "list":
                for sub in block.get("blocks", []):
                    parts.extend(sub.get("text", {}).get("content", []))
        plain_text = "\n".join(parts)[:6000]

    try:
        resp = await seller.update_description(
            product_id=p.sku, description=plain_text, rich_content_json=rich,
        )
    except SellerAPIError as e:
        raise HTTPException(502, f"Ozon отклонил: {e.message[:200]}")
    await log_action(db, action="rich_publish", user_id=user.id,
                     entity_type="product", entity_name=p.name or p.sku,
                     details={"sku": p.sku, "blocks": len(rich.get("content", []))})
    return {"ok": True, "sku": p.sku, "ozon_response": resp}